...
Model* modeloPalancaGlobal = nullptr;
Model* modeloPalanca2Global = nullptr;
...
Escenario::Escenario() {
    ...
    "The shipment that arrived had a strange smell.\nDoesn't seem to be anything serious.\n\n"
    "-Employee Report",
    3.0f);
...
bool Escenario::togglePuertaMadera(glm::vec3 jugadorPos) {
    bool algunaToggled = false;
    ...
    if (monstruoManager && monstruoManager->isMonstruoActivo()) {
        ...
        for (auto& puerta : puertasMadera) {
            if (puerta.jugadorCerca(jugadorPos)) {
                if (puerta.toggle()) algunaToggled = true;
            }
        }
...
void Escenario::PasilloBodega() {
    ...
    cartelExit.color = glm::vec3(1.0f);
    cartelExit.tieneColision = false;
    cartelExit.esPuerta = false;
    cartelExit.tieneTextura = true;
    cartelExit.texturaID = texturaCartel;
...
void Escenario::togglePalanca() {
    ...
    if (palancaActivada) {
        if (modeloPalanca2Ptr) {
            for (size_t i = 0; i < modelosExtra.size(); i++) {
                if (modelosExtra[i] == modeloPalancaPtr) {
                    modelosExtra[i] = modeloPalanca2Ptr;
                    std::cout << "🔄 Palanca cambiada a estado ACTIVADA (lever2)" << std::endl;
                    break;
                }
            }
        }
        ...
        for (auto& puerta : puertasMadera) {
            glm::vec3 posPuerta = puerta.getPosicion();
            if (posPuerta.x > 35.8f && posPuerta.x < 36.2f &&
                posPuerta.z > 35.3f && posPuerta.z < 35.7f) {
                puerta.setBloqueada(true);
                std::cout << "🔒 Puerta del pasillo frontal bloqueada permanentemente" << std::endl;
            }
        }

        for (auto& puerta : puertasMadera) {
            glm::vec3 posPuerta = puerta.getPosicion();
            if (posPuerta.x > 27.3f && posPuerta.x < 27.7f &&
                posPuerta.z > 36.8f && posPuerta.z < 37.2f) {
                puerta.setBloqueada(true);
                std::cout << "🔒 Puerta del área de seguridad bloqueada permanentemente" << std::endl;
            }
        }
...
void Escenario::PasillosAreaSeguridad() {
    ...
    // ===== MODELOS DE PALANCA =====
    // Usamos variables globales declaradas en Escenario.cpp
    if (!modeloPalancaGlobal) {
        modeloPalancaGlobal = new Model("models/crowbar/lever.obj");
        modeloPalancaGlobal->position = glm::vec3(37.5f, sueloY + 3.0f, 37.0f);
        modeloPalancaGlobal->rotation = glm::vec3(0.0f, 0.0f, 0.0f);
        modeloPalancaGlobal->scale = glm::vec3(1.0f, 1.0f, 1.0f);
        std::cout << "Palanca agregada en el pasillo frontal" << std::endl;
        modeloPalancaPtr = modeloPalancaGlobal;
    }

    if (!modeloPalanca2Global) {
        modeloPalanca2Global = new Model("models/crowbar/lever2.obj");
        modeloPalanca2Global->position = glm::vec3(37.5f, sueloY + 3.0f, 37.0f);
        modeloPalanca2Global->rotation = glm::vec3(0.0f, 0.0f, 0.0f);
        modeloPalanca2Global->scale = glm::vec3(1.0f, 1.0f, 1.0f);
        std::cout << "Palanca activada (lever2) cargada" << std::endl;
        modeloPalanca2Ptr = modeloPalanca2Global;
    }
...
void Escenario::PasilloRecepcion() {
    ...
    float zModelos = zSegundaPuerta - 1.5f;

    static Model* Maquina = nullptr;
    if (!Maquina) {
        Maquina = new Model("models/MaquinaExpendedora/MaquinaExpendedora.obj");
        Maquina->position = glm::vec3(xCentro - 3.7f, sueloY, zModelos);
        Maquina->rotation = glm::vec3(0.0f, -90.0f, 0.0f);
        Maquina->scale = glm::vec3(2.0f, 2.0f, 2.0f);
        std::cout << "Maquina expendedora agregada" << std::endl;
    }
    modelosExtra.push_back(Maquina);
    modelosPosiciones.push_back(glm::vec3(0.0f, 0.0f, 0.0f));
    modelosEscalas.push_back(glm::vec3(1.0f, 1.0f, 1.0f));
    ...
    if (!Maceta) {
        Maceta = new Model("models/Maceta/Maceta.obj");
        Maceta->position = glm::vec3(xCentro + 3.5f, sueloY, zModelos);
        Maceta->rotation = glm::vec3(0.0f, 0.0f, 0.0f);
        Maceta->scale = glm::vec3(1.0f, 1.0f, 1.0f);
        std::cout << "Maceta agregada" << std::endl;
    }
    modelosExtra.push_back(Maceta);
    modelosPosiciones.push_back(glm::vec3(0.0f, 0.0f, 0.0f));
    modelosEscalas.push_back(glm::vec3(1.0f, 1.0f, 1.0f));

    ObjetoFisico hitboxMaceta;
    hitboxMaceta.posicion = glm::vec3(xCentro + 3.5f, sueloY, zModelos);
    hitboxMaceta.escala   = glm::vec3(0.8f, 5.2f, 0.8f);
    hitboxMaceta.tieneColision = true;
...
void Escenario::reset() {
    ...
    // ===== 3. REINICIAR PUERTAS BLOQUEADAS =====
    puertaJefeBloqueada = true;
    puertaSalidaBloqueada = true;
    botonSalidaPresionado = false;

    // ===== 4. REINICIAR PUERTAS DE MADERA =====
    for (auto& puerta : puertasMadera) {
        puerta.setBloqueada(false);
        // Si tienes método setAbierta, usarlo
        // puerta.setAbierta(false);
    }

    // ===== 5. REINICIAR INDICADORES DE PROGRESO =====
    actualizarIndicadoresProgreso();

    // ===== 6. REINICIAR PALANCA VISUAL =====
    for (size_t i = 0; i < modelosExtra.size(); i++) {
        if (modelosExtra[i] == modeloPalanca2Global) {
            modelosExtra[i] = modeloPalancaGlobal;
            std::cout << "🔄 Palanca visual reseteada a lever.obj" << std::endl;
            break;
        }
    }

    // ===== 7. REINICIAR MONSTRUO =====
    monstruoActivado = false;
    if (monstruoManager) {
        monstruoManager->resetear();
    }
